...
\newcommand{\expThreeErrorFig}{
	\begin{figure}
		...
		\caption{From experiment 3, the absolute error in estimations of trend defined in two ways: either as difference from the robust slope (which excludes outliers), or the OLS slope (which does not). Participants were significantly closer to the robust fit, indicating that they were largely insensitive to outliers. We include the error from trials with no outliers (and so where the two methods of calculating error agree) for reference. Error to the robust line is higher than this standard, indicating that participants were at least performing some interpolation between OLS and robust fits, even if they largely favored the robust fit. Confidence intervals represent bootstrapped 95\% c.i.s of interquartile mean. }
